...
int main() {
    ...
    if (direction == -1) {
        ...
        wallFollowStop = false;
        const float ARC_RADIUS_CM = 30.0f;
        ...
        for (int lap = 0; lap < 10; ++lap) {
            ...
            // Perform the arc first
            arc90Back(1, 40);  // blocking, completes fully
            std::this_thread::sleep_for(std::chrono::milliseconds(200)); // cooldown
            arcInProgress = false;
            ...
            wallFollowStop = false;
            wallFollowThread = std::thread([&]() {
                followLeftWallRearStableYaw(20.0f, -40, 1.0f, 0.0f, 0.5f, 10.0f);
            });
        ...
        // Wall-follow toward starting position
        wallFollowStop = false;
        const float ARC_RADIUS_CM = 40.0f;
        const float leftThreshold = Li + ARC_RADIUS_CM;
        ...
        while (!reachedLeft && !ctrl_c_pressed) {
            float frontDist = readLIDAR_median_cm(latest_points, 80.0f, 100.0f);

            if (frontDist <= leftThreshold) {
                reachedLeft = true;
                if (!arcInProgress) {
                    wallFollowStop = true;
                    sendCommand("M_STOP");
        ...
        wallFollowStop = false;
        wallFollowThread = std::thread([&]() {
            followLeftWallRearStableYaw(30.0f, -40, 1.0f, 0.0f, 0.5f, 10.0f);
        });

        const float frontThreshold = Fi + 20.0f;
        bool reachedFront = false;
        while (!reachedFront && !ctrl_c_pressed) {
            float frontDist = readLIDAR_median_cm(latest_points, 80.0f, 100.0f);
