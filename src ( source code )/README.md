...
```mermaid
flowchart TD
    Start --> Init[Initialize LIDAR+IMU+I²C]
    Init --> Measure[Measure Initial Distances]
    Measure --> Decide[Decide Wall Side]
    Decide --> Approach[Move Backward to Wall]
    Approach --> LapLoop{Lap Loop (×3)}
    LapLoop -->|Wall-follow| WallFollow[PID Wall-Follow (Thread)]
    WallFollow -->|Detect Front| Obstacle[Stop & 90° Arc]
    Obstacle -->|Next Lap| LapLoop
